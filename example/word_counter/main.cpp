...
int main() {
    ...
    for (int i = 1; i < 5; i++) {
        map_input_files.emplace_back(
                home_dir / ("distributed_map_reduce/example/word_counter/data/input_" + std::to_string(i) + ".csv")
        );
    }
    fs::path dll_path = home_dir / "distributed_map_reduce/example/word_counter/build/libmap_reduce_config.so";
